...
sim_inf10_bruf <- function(linha = "Regi\u00e3o", coluna = "N\u00e3o ativa", conteudo = 1, periodo = "last", regiao = "all",
  ...
  page <- xml2::read_html("http://tabnet.datasus.gov.br/cgi/deftohtm.exe?sim/cnv/inf10uf.def")
  ...
  causa_mal_definidas.df <- suppressWarnings(data.frame(id = page %>% rvest::html_nodes("#S6 option") %>% rvest::html_text() %>% trimws(),
  ...
  causas_evitaveis_0a4anos.df <- data.frame(id = page %>% rvest::html_nodes("#S7 option") %>% rvest::html_text() %>% trimws(),
  ...
  obito_investigado.df <- data.frame(id = page %>% rvest::html_nodes("#S24 option") %>% rvest::html_text() %>% trimws(),
  ...
  regiao.df$id[1] <- unidade_da_federacao.df$id[1] <- capitulo_cid10.df$id[1] <- categoria_cid10.df$id[1] <- "all"
  lista_mort_cid10.df$id[1] <- causa_mal_definidas.df$id[1] <- causas_evitaveis_0a4anos.df$id[1] <- "all"
  faixa_etaria_1.df$id[1] <- faixa_etaria_2.df$id[1] <- faixa_etaria_3.df$id[1] <- faixa_etaria_4.df$id[1] <- "all"
  faixa_etaria_5.df$id[1] <- faixa_etaria_detalhada.df$id[1] <- sexo.df$id[1] <- cor_raca.df$id[1] <- "all"
  ...
  form_data <- paste(
    form_linha, form_coluna, form_conteudo, form_periodo, form_pesqmes1, form_regiao,
    form_unidade_da_federacao, form_pesqmes3, form_capitulo_cid10, form_pesqmes4,
    form_categoria_cid10, form_pesqmes5, form_lista_mort_cid10, form_causa_mal_definidas,
    form_causas_evitaveis_0a4anos, form_faixa_etaria_1, form_pesqmes9, form_faixa_etaria_2,
    form_faixa_etaria_3, form_faixa_etaria_4, form_faixa_etaria_5, form_pesqmes13,
    form_faixa_etaria_detalhada, form_sexo, form_cor_raca, form_local_ocorrencia,
    form_pesqmes17, form_idade_mae, form_escolaridade_mae, form_pesqmes19,
    form_duracao_gestacao, form_tipo_gravidez,   form_tipo_parto, form_peso_ao_nascer,
  ...
  site <- httr::POST(url = "http://tabnet.datasus.gov.br/cgi/tabcgi.exe?sim/cnv/inf10uf.def",
                     body = form_data)
