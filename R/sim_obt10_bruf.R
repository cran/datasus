...
sim_obt10_bruf <- function(linha = "Regi\u00e3o", coluna = "N\u00e3o ativa", conteudo = 1, periodo = "last", regiao = "all",
  ...
  page <- xml2::read_html("http://tabnet.datasus.gov.br/cgi/deftohtm.exe?sim/cnv/obt10uf.def")
  ...
  grupo_cid10.df <- data.frame(id = page %>% rvest::html_nodes("#S4 option") %>% rvest::html_text() %>% trimws(),
  ...
  causa_br_cid10.df <- data.frame(id = page %>% rvest::html_nodes("#S6 option") %>% rvest::html_text() %>% trimws(),
  ...
  causa_mal_definida.df <- data.frame(id = page %>% rvest::html_nodes("#S7 option") %>% rvest::html_text() %>% trimws(),
  ...
  faixa_etaria.df <- data.frame(id = page %>% rvest::html_nodes("#S8 option") %>% rvest::html_text() %>% trimws(),
                                value = page %>% rvest::html_nodes("#S8 option") %>% rvest::html_attr("value"))
  faixa_etaria.df[] <- lapply(faixa_etaria.df, as.character)

  faixa_etaria_ops.df <- data.frame(id = page %>% rvest::html_nodes("#S9 option") %>% rvest::html_text() %>% trimws(),
                                    value = page %>% rvest::html_nodes("#S9 option") %>% rvest::html_attr("value"))
  faixa_etaria_ops.df[] <- lapply(faixa_etaria_ops.df, as.character)

  faixa_etaria_det.df <- data.frame(id = page %>% rvest::html_nodes("#S10 option") %>% rvest::html_text() %>% trimws(),
                                    value = page %>% rvest::html_nodes("#S10 option") %>% rvest::html_attr("value"))
  faixa_etaria_det.df[] <- lapply(faixa_etaria_det.df, as.character)

  faixa_etaria_menor1a.df <- data.frame(id = page %>% rvest::html_nodes("#S11 option") %>% rvest::html_text() %>% trimws(),
  ...
  regiao.df$id[1] <- unidade_da_federacao.df$id[1] <- local_ocorrencia.df$id[1]<- capitulo_cid10.df$id[1] <- "all"
  causa_br_cid10.df$id[1] <- causa_mal_definida.df$id[1] <- grupo_cid10.df$id[1] <- categoria_cid10.df$id[1] <- "all"
  faixa_etaria_det.df$id[1] <- faixa_etaria_menor1a.df$id[1] <- faixa_etaria.df$id[1] <- faixa_etaria_ops.df$id[1] <- "all"
  estado_civil.df$id[1] <- sexo.df$id[1] <- cor_raca.df$id[1] <- escolaridade.df$id[1] <- "all"
  ...
  form_data <- paste(form_linha, form_coluna, form_conteudo, form_periodo, form_regiao,
                     form_pesqmes2, form_unidade_da_federacao, form_pesqmes3, form_capitulo_cid10,
                     form_pesqmes4, form_grupo_cid10, form_pesqmes5, form_categoria_cid10, form_pesqmes6,
                     form_causa_br_cid10, form_causa_mal_definida, form_pesqmes8, form_faixa_etaria,
                     form_pesqmes9, form_faixa_etaria_ops, form_pesqmes10, form_faixa_etaria_det,
                     form_faixa_etaria_menor1a, form_sexo, form_cor_raca, form_estado_civil,
                     form_local_ocorrencia, "formato=table&mostre=Mostra", sep = "&")

  form_data <- gsub("\\\\u00", "%", form_data)

  ##### REQUEST FORM AND DATA WRANGLING ####
  site <- httr::POST(url = "http://tabnet.datasus.gov.br/cgi/tabcgi.exe?sim/cnv/obt10uf.def",
                     body = form_data)
